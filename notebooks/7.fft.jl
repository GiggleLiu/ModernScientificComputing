...
# ╔═╡ 90fbe80c-dd4d-45c8-9ece-851392532aa3
using LinearAlgebra
...
md"## Application 2: Wavelets"

# ╔═╡ df89ee5b-c0df-41d7-9618-b80dcc7a823e
function gaussian_packet(x0, Δx)
    return x-> exp((x - x0) ^ 2 / (-2*Δx^2))
end

# ╔═╡ 10561a2d-32c5-4360-942e-a60b9791beb8
let
	f = gaussian_packet(200, 20)
	y = f.(1:500)
	plot(abs.(y); xlabel="x", ylabel="amplitude", label="")
end

# ╔═╡ efbde06c-c892-4d49-8c48-ae21d1bbe8da
function fourier_transform(f, xs, u::Real)
	sum(x->exp(-2π*im*u*x) * f(x), xs)
end

# ╔═╡ 1c8830db-b0f6-4a81-9f3b-2f533af62cbf
fourier_transform(gaussian_packet(200, 20), 0:1:500, 20)

# ╔═╡ ab543b02-4af1-47b0-ade3-d429e7cf182f
plot(abs.(fft(gaussian_packet(200, 20).(0:499))))

# ╔═╡ 4c732a90-b73d-46c9-aab7-5b4573b61744
let
	k = -0.5:0.01:0.5
	# we use `circshift` to shift k = 0 to middle of the vector.
	y = fourier_transform.(gaussian_packet(200, 20), Ref(0:1:499), k)
	plot(k, abs.(y); xlabel="k", ylabel="F(k)", label="", ylim=(0, 50))
end
...
let
	k = 0:127
	# we use `circshift` to shift k = 0 to middle of the vector.
	y = circshift(fourier_transform.(x->cos(π * x), Ref(0:1/128:0.99), k), 64)
	plot(k, abs.(y); xlabel="k", ylabel="F(k)", label="", ylim=(0, 50))
end

# ╔═╡ 41401216-2a29-4bd5-a8f9-13defceab4ff
let
	x = 0:127
	f = cos.(π .* x ./ 127)
	F = fft(f)
	k = 0:127
	plot(k, abs.(F); xlabel="k", ylabel="F(k)", label="", ylim=(0, 50))
end
...
img_data_k = fftshift(fft(img_data)) ./ sqrt(prod(size(img_data)))

# ╔═╡ 820f7c11-92e5-4123-b778-8b79c7f54677
Gray.(abs2.(img_data_k))
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "05650ec372078b81fd1b8f3eb7e0cf77494163fd"
